...
class Translator(TranslatorInterface):
    def __init__(self, indices, app_client_id, app_client_secret, token):
        ...
        self.token = token
        self.index = self.DEFAULT_INDEX_WITHOUT_PREFIX
    ...
    def update(self, entity_id, document):
        ...
        print('{} {}'.format(entity_id, document))

        #for index in self.indices.keys():
        public_index = self.INDICES['indices'][self.index]['public']
        private_index = self.INDICES['indices'][self.index]['private']

        logger.debug('private_index: '.format(private_index))
